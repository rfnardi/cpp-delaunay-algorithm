...
#include "./Vizinhanca.hpp"
  ...
  del::Triangle del::Triangle_Building::Point_based_Triangle_Building (del::Point A)
  {
    ...
    do
    {
      ...
      for (size_t linha_A = 1; linha_A < this->Catalogo_Viz[VIZ_A].size(); linha_A++)
      {
        for (size_t col_A = 0; col_A < 3; col_A++)
        {
          P1.p[col_A] = this->Catalogo_Viz[VIZ_A][linha_A].p[col_A];
          for (size_t linha_B = 1; linha_B < this->Catalogo_Viz[VIZ_B].size(); linha_B++)
          {
            for (size_t col_B = 0; col_B < 3; col_B++)
            {
              P2.p[col_B] = this->Catalogo_Viz[VIZ_B][linha_B].p[col_B];
  ...
  del::Triangle del::Triangle_Building::Edge_based_Triangle_Building (del::Edge edge, del::Triangle Base_triangle)
  {
    ...
    do
    {
      ...
      del::Point P2;
      del::Point C;
      for (size_t linha_A = 0; linha_A < this->Catalogo_Viz[VIZ_A].size(); linha_A++)//BUSCA DO PONTO C
      {
