  ...
  // ********************* início do bloco com y enquanto direção líder *************************
  ...
  // ********************* fim do bloco com y enquanto direção líder ****************************
  // ********************************************************************************************
  // ********************* início do bloco com x enquanto direção líder *************************


  if (keeping_track_of_which_dimension[0] = 0) // se y for a dimensão líder
  {
    std::cout << "Direção líder: y" << '\n';
    ...
    while(l<number_of_points)
    {
      ...
      while(sub_chart_index < number_of_sub_charts && region_index < number_of_re_shaped_regions)
      {
        ...
        if (size_regions_rate[region_index] == 0.0)//happens when the number of points doesn't exceed the max per chart
        {
          if (sub_chart_index != (number_of_sub_charts - 1) && x[l]>=mins_and_maxs[2] + sum_sizes(region_index, sizes_and_fusions + number_of_re_shaped_regions)*step1 && x[l]< mins_and_maxs[2] + (sum_sizes(region_index + 1, sizes_and_fusions + number_of_re_shaped_regions))*step1)
          {
            Index_Vector[region_index]++;
            New_Index_Vector[sub_chart_index]++;
            //std::cout << "Em sub_charts_building. Encontrou ponto em carta secundária: region_index = " << region_index << " ; sub_chart_index = "<< sub_chart_index << '\n';
          ...
          if (sub_chart_index == (number_of_sub_charts - 1) && x[l]>=mins_and_maxs[2] + sum_sizes(region_index, sizes_and_fusions + number_of_re_shaped_regions)*step1 && x[l]<= mins_and_maxs[3] )
          {
            Index_Vector[region_index]++;
            New_Index_Vector[sub_chart_index]++;
            //std::cout << "Em sub_charts_building. Encontrou ponto em carta secundária: region_index = " << region_index << " ; sub_chart_index = "<< sub_chart_index << '\n';
        ...
        if (size_regions_rate[region_index] != 0.0)
        {
          ...
          for (size_t b = 0; b < (1 + (int) size_regions_rate[region_index]); b++)
          {
            if (sub_chart_index != (number_of_sub_charts - 1) && x[l]>=mins_and_maxs[2] + sum_sizes(region_index, sizes_and_fusions + number_of_re_shaped_regions)*step1 && x[l]< mins_and_maxs[2] + (sum_sizes(region_index + 1, sizes_and_fusions + number_of_re_shaped_regions))*step1)
            {
              if (b != (int) size_regions_rate[region_index] && y[l]>=mins_and_maxs[0]+ b*step2 && y[l]< mins_and_maxs[0] + (b + 1)*step2)
              {
                New_Index_Vector[sub_chart_index]++;
                //std::cout << "Em sub_charts_building. Encontrou ponto em carta terciária: region_index = " << region_index << " ; sub_chart_index = " << sub_chart_index << " ; b = " << b << '\n';
                jump_out = true;
              ...
              if (b == (int) size_regions_rate[region_index] && y[l]>=mins_and_maxs[0]+ b*step2 && y[l]<= mins_and_maxs[1])
              {
                New_Index_Vector[sub_chart_index]++;
                //std::cout << "Em sub_charts_building. Encontrou ponto em carta terciária: region_index = " << region_index << " ; sub_chart_index = " << sub_chart_index << " ; b = " << b << '\n';
                jump_out = true;
  ...
  // ********************* fim do bloco com x enquanto direção líder ****************************
